...
class ChangelogUpdater(DocumentUpdater):
    ...
    def apply(
        ...
        entries = docs.get("changelog_entries", [])
        changed = self._update_changelog(path, entries)
        return UpdateResult(changed=changed, path=path, updater_name=self.name)

    def _update_changelog(self, path: Path, entries: list[str]) -> bool:
...
def update_changelog(path: Path, entries: list[str]) -> bool:
    ...
    return updater._update_changelog(path, entries)
