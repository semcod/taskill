...
> **Recently shipped (manual note):** autonomous task execution via koru/coru.
> Bare `taskill` (or `taskill koru`) resolves pending tasks from a list
> (TODO checkboxes → planfile tickets → `TASK.md`) and runs each through
> `coru text "<task>" --llm`. See the "Autonomous execution (koru)" section in
> the README and the CHANGELOG `[Unreleased]` entry.
>
> Follow-ups to consider:
> - Optional MCP/A2A transport (talk to `mcp2koru`/`koruapi` instead of shelling
>   out to the `coru` CLI).
> - Per-task result reporting back into `planfile.yaml` ticket status.
> - Parallel task execution with a concurrency cap.
