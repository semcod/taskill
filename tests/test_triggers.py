...
def _commit(subject: str = "feat: x") -> Commit:
    return Commit(sha="0", short_sha="0", author="x", date="2026-01-01T00:00:00Z", subject=subject)
...
def test_recent_run_skips_when_no_commits(tmp_path: Path) -> None:
    triggers = Triggers(
        min_hours_since_last_run=24, min_commits_since_last_run=1, changed_files_threshold=1
    )
...
def test_commits_above_threshold_triggers(tmp_path: Path) -> None:
    triggers = Triggers(
        min_hours_since_last_run=24, min_commits_since_last_run=2, changed_files_threshold=1
    )
    state = TaskillState(last_run_iso=datetime.now(timezone.utc).isoformat())
...
def test_coverage_delta_triggers(tmp_path: Path) -> None:
    triggers = Triggers(
        min_hours_since_last_run=999,
        min_commits_since_last_run=999,
        changed_files_threshold=999,
        coverage_change_pct=2.0,
    )
    state = TaskillState(
...
def test_sumd_change_triggers(tmp_path: Path) -> None:
    triggers = Triggers(
        min_hours_since_last_run=999, min_commits_since_last_run=999, changed_files_threshold=999
    )
    state = TaskillState(
        last_run_iso=datetime.now(timezone.utc).isoformat(),
